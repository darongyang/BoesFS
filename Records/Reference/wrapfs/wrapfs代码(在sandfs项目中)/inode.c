...
// 创建一个文件时会调用，分别完成底层和wrapfs层的inode创建、dentry和inode的绑定
			 ...
			 umode_t mode, bool want_excl)
{
	...
	wrapfs_get_lower_path(dentry, &lower_path); // 获得dentry的底层path
	lower_dentry = lower_path.dentry; // 获得dentry的底层dentry
	lower_parent_dentry = lock_parent(lower_dentry); // 父目录底层dentry

	err = vfs_create(d_inode(lower_parent_dentry), lower_dentry, mode,
			 want_excl); // vfs_create调底层fs的方法，底层dentry和inode绑定
	if (err)
		goto out;
	err = wrapfs_interpose(dentry, dir->i_sb, &lower_path); // 将lower_path对应的wrapfs inode和dentry关联
															// wrapfs层dentry和inode绑定
	if (err)
		goto out;
	fsstack_copy_attr_times(dir, wrapfs_lower_inode(dir)); // 更新wrapfs的父目录inode dir的属性
