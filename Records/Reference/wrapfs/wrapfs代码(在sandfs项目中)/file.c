			   ...
			   size_t count, loff_t *ppos)
{
	...
	lower_file = wrapfs_lower_file(file); // 获取底层file
	err = vfs_read(lower_file, buf, count, ppos); // 调用vfs_read读底层file
	/* update our inode atime upon a successful lower read */
	if (err >= 0)
		fsstack_copy_attr_atime(d_inode(dentry),
					file_inode(lower_file)); // 更新wrapfs file对应inode的atime

	return err;
}

// wrapfs写file的方法 调vfs_write
			    ...
			    size_t count, loff_t *ppos)
{
	...
	lower_file = wrapfs_lower_file(file); // 获取底层file
	err = vfs_write(lower_file, buf, count, ppos); // 调用vfs_write写底层file
	/* update our inode times+sizes upon a successful lower write */
	if (err >= 0) {
		fsstack_copy_inode_size(d_inode(dentry), // 更新wrapfs file对应inode的size和times
...
// wrapsfs的mmap
// 异常处理 -> 调用底层的mmap -> 设置wrapfs file的aop，同时vma_op被替换为wrapfs的，原先底层fs的被save到file info
static int wrapfs_mmap(struct file *file, struct vm_area_struct *vma)
{
	 ...
	 */
	file_accessed(file);
	vma->vm_ops = &wrapfs_vm_ops; // vma_op被替换为wrapfs的

	file->f_mapping->a_ops = &wrapfs_aops; /* set our aops */ // wrapfs file的aop
	if (!WRAPFS_F(file)->lower_vm_ops) /* save for our ->fault */
		WRAPFS_F(file)->lower_vm_ops = saved_vm_ops; // 原先底层fs的vma_op被save到file info
...
// 打开文件时，wrapfs file的初始化操作
// 异常处理 -> 通过low path打开文件，获取low file -> 将low file和wrapfs file绑定即是初始化wrapfs file -> 最后复制一下inode属性
static int wrapfs_open(struct inode *inode, struct file *file)
{
	...
	lower_file = dentry_open(&lower_path, file->f_flags, current_cred()); // 通过low path 打开low file
	path_put(&lower_path);
	if (IS_ERR(lower_file)) {
		...
		wrapfs_set_lower_file(file, lower_file); // 将打开的low file和wrapfs file绑定即是打开wrapfs file
	}

	if (err)
		kfree(WRAPFS_F(file));
	else
		fsstack_copy_attr_all(inode, wrapfs_lower_inode(inode)); // 复制一下inode属性
out_err:
	return err;
}
...
static int wrapfs_file_release(struct inode *inode, struct file *file)
{
	...
	lower_file = wrapfs_lower_file(file);
	if (lower_file) {
		wrapfs_set_lower_file(file, NULL); // 取消绑定
		fput(lower_file); // 减少文件引用
	}

	kfree(WRAPFS_F(file)); // 释放file info
	return 0;
}
