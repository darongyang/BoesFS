...
static int sandfs_unlink(struct inode *dir, struct dentry *dentry)
{
    ...
    // //  int PATH_MAX_SUPP = 256;
    // //  采集path参数
    // char path_buf[256] = {0};
	// //  我还不知道该怎么获取到全局的路径. 目前感觉是只能获取到的以挂载点为根目录的路径
    // char *fn = dentry_path_raw(dentry, path_buf,256);
    // //  所以我在这里先写死看，假定一定挂载到/home/fs
	// char path_whole[256] = "/home/fs";    
    // strcat(path_whole,fn);
    // struct sandfs_args args;
    // //  存储路径
    // args.args[0].size = strlen(path_whole);
	// args.args[0].value = (void *)path_whole;
    // args.num_args = 1;
    // args.op = SANDFS_UNLINK;
    // //  检测参数
