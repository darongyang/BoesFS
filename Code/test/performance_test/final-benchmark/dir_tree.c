...
{
    // printk("============== divide_name start path %s==============\n",path);
...
{
    //  struct Node *child = node->children[tmp_name];
    //  return child;
    //  遍历node节点的 指向的 子红黑树
    // printk("rb tree root %p %s ; target %s %d\n", node,node->val,tmp_name,name_len);
    //  never happened
    if(node == NULL) {
    ...
    while (cur_node) {                                      //  struct Node *child = node->children[tmp_name];
        ...
        // printk("cur dir node val = %s ; val_len = %d\n",t->val,t->val_len);
...
struct dir_node *query_dir_tree(struct dir_node* dir_tree_root,const char *path)
{
    ...
    {
        // printk("query_dir_tree tmp_name %s\n",tmp_name);
...
int insert_dir_tree(struct dir_node* dir_tree_root,const char *path , policy_t rule)
{
    ...
    while(tmp_name && (*tmp_name)!='\0')
    {
        ...
        // printk("insert_dir_tree tmp name %s tmp_len = %d\n",tmp_name,tmp_len);
        // pr_info("tmp_name = %s , tmp_len = %d\n",tmp_name,tmp_len);
        ...
        if(cur == NULL)
        {
            ...
            // pr_info("name = %s , node->rule.valid = %d\n",tmp_name,node->rule.valid);
            //  包含了 insert 根节点（parent为空）的情况
            if(rb_insert(parent,node,tmp_len)) {
                //  cur 指向node. 下一轮继续向下遍历
                cur = node;
                // if(last) {
                //     printk("target node %s insert successfully!\n",tmp_name);
                // } else {
                //     printk("path node %s insert successfully!\n",tmp_name);
                // }
            } else {
...
int destroy_dir_tree(struct dir_node* dir_tree_root)
{
    ...
    // printk("destroy dir tree successfully\n");
    return TRUE;
}
