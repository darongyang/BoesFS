...
static void deleteEnter(char *string)
{
    int i = 0;
    while(i < PATH_MAX_SUPP && string[i] != '\n' && string[i] != '\0')
        i++;
    if(i == PATH_MAX_SUPP)
    {
        string[i - 1] = '\0';
...
static void getlistfromfile(ebpf_context_t *ctx, char *file, int map_idx, int *val)
{
    ...
    while(!feof(fp))
    {
        fgets(key, PATH_MAX_SUPP, fp);
        deleteEnter(key);
        ebpf_data_lookup(ctx, (void *)key, tmp, map_idx);
        if (tmp == NULL)
            ebpf_data_update(ctx, (void *)key, (void *)val, map_idx, 0);
        else
        {
            int new_val;
            new_val = *val | *((int *)tmp);
            ebpf_data_update(ctx, (void *)key, (void *) &new_val, map_idx, 0);
        }
    }
    fclose(fp);
    return;
...
main(int argc, char *argv[])
{
	...
	if (prog) {
        ...
        int map_idx = 0;
        int val = OPEN_DENY;
        // getlistfromfile(ctx, "/home/fs/fs_noopen.txt", map_idx, &val);
        val = READ_DENY;
        getlistfromfile(ctx, "/home/fs/fs_noread.txt", map_idx, &val);
        val = WRITE_DENY;
        getlistfromfile(ctx, "/home/fs/fs_nowrite.txt", map_idx, &val);
        val = LOOKUP_DENY;
        getlistfromfile(ctx, "/home/fs/fs_noaccess.txt", map_idx, &val);
	}
